...
impl VulkanContext {
    pub fn new(name: &str) -> Result<Self, Box<dyn Error>> {
        ...
        let surface_khr = unsafe {
            ...
            let android_native_window::DisPlayInfo {
                orientation: _,
                width,
                height,
            } = android_native_window::safe_get_display_info();
            let res = if width > height { width } else { height };
            window_handle.a_native_window =
                safe_create_native_window(name, res as i32, res as i32, true);
